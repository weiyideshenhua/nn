...
def gaussian_basis(x, feature_num=10):
    ...
    参数：
    x (ndarray): 输入数据，形状为 (N,)，即有 N 个数据点。
    feature_num (int): 高斯基函数的数量，默认值为 10。

    返回：
    ndarray: 形状为 (N, feature_num) 的矩阵，其中每个元素表示对应输入 x 对应的高斯基函数响应。
    示例：假设 x 是一个包含 5 个数据点的数组，计算它们对应的高斯基函数响应
    x = np.array([1, 5, 10, 15, 20])
    result = gaussian_basis(x)
    print(result)
    """
    # 定义中心在区间 [0, 25] 内均匀分布
    ...
    # 对于每个 x，计算它和所有中心点的距离，距离越近，高斯响应越大
